...
<nav id="menu">
	...
	<!-- 아이디가 일치하면 -->
	<c:choose>
		<c:when test="${id eq board.id || id == 'admin1234'}">
			<div class="out">
				...
				<input class="button" type="button" value="목록" onclick="location.href='petList.do?pageNum=${pageNum}'">	
			</div>
		</c:when>
		<c:otherwise>
			<div class="out"><input class="button" type="button" value="목록" onclick="location.href='petList.do?pageNum=${pageNum}'"></div>
		</c:otherwise>
	</c:choose>
		...
		<form action="petCommWritePro.do?pageNum=${pageNum}&bd_num=${bd_num}&comm_id=${id}&board_id=${board.id }&commPageNum=${commPageNum }">
		...
		<c:if test="${commTotCnt>0 }">
			<c:forEach var="commList" items="${commList }">
				<div class="in">
					...
					<div style="width:200px; text-align:right; vertical-align:baseline; display:inline-block;"><font size="2px"><a href="petCommLike.do?bd_num=${bd_num}&board_id=${board.id }&comm_id=${commList.comm_id }&pageNum=${pageNum }&type_num=100&comm_num=${commList.comm_num }&commPageNum=${commPageNum }"><img alt="image" style="width:12px; height:12px; object-fit:cover;" src="css_img/like.png"></a> ${commList.comm_like }</font></div>																																											
					</div>	
				</div>
				...
				<c:choose>
				<c:when test="${commList.comm_id == id || id == 'admin1234'}">	
						<div style="width:750px; display:inline-block;">
						<input class="button" type="button" value="수정" onclick="location.href='petCommUpdate.do?bd_num=${bd_num}&pageNum=${pageNum}&type_num=${type_num}&comm_num=${commList.comm_num}&commPageNum=${commPageNum }'">
						<input class="button" type="button" value="삭제" onclick="location.href='petCommDelete.do?bd_num=${bd_num}&pageNum=${pageNum }&board_id=${board.id }&type_num=100&comm_num=${commList.comm_num }&commPageNum=${commPageNum }'">
						<hr><!-- 구분선 -->
						</div>
				</c:when>
				<c:otherwise>
		...
		<c:if test="${commTotCnt > 0 }">
			<div>
				<c:if test="${commStartPage > commBlockSize }">
					<a href="petContent.do?commPageNum=${commStartPage - commBlockSize }&bd_num=${bd_num}&type_num=100&pageNum=${pageNum}" style="font-size:13px;">[이전]</a>
				</c:if>
				<c:forEach var="i" begin="${commStartPage }" end="${commEndPage }" >
					<a href="petContent.do?commPageNum=${i }&bd_num=${bd_num}&type_num=100&pageNum=${pageNum}" style="font-size:13px;">[${i}]</a>
				</c:forEach>
				<c:if test="${commEndPage < commPageCnt }">
					<a href="petContent.do?commPageNum=${commStartPage + commBlockSize }&bd_num=${bd_num}&type_num=100&pageNum=${pageNum}" style="font-size:13px;">[다음]</a>
				</c:if>
			</div>
